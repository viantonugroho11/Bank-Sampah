    ...
    <table id="example" class="display" cellspacing="0" width="100%" border="0">
        ...
        while ($row = mysqli_fetch_assoc($query)) { ?>
            <tr align="center">
                ...
                <td>
                    <?php
              $saldonya = mysqli_query(
                  $conn,
                  "SELECT SUM(total) AS totalsaldo FROM setor WHERE nin='" .
                      $row['nin'] .
                      "'"
              );
              $tariknya = mysqli_query(
                  $conn,
                  "SELECT SUM(jumlah_tarik) AS totaltarik FROM tarik WHERE nin='" .
                      $row['nin'] .
                      "'"
              );
              ...
              $tot_saldo_total =
                  $var_saldo['totalsaldo'] - $var_tarik['totaltarik'];
              $saldoakhir = mysqli_query(
                  $conn,
                  "update nasabah SET saldo=$tot_saldo_total WHERE nin='" .
                      $row['nin'] .
                      "'"
              );
              ?>
                ...
                <td>
                    ...
                    <a onclick="return confirm('Anda yakin ingin menghapus data ini?')" href="../system/function/delete-nasabah.php?id=<?php echo $row[
                    'nin'
                ]; ?>">
