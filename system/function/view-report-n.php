...
Highcharts.chart('container', {
    ...
    xAxis: {
        ...
        categories: [
            <?php
  $query = mysqli_query($conn, 'SELECT * FROM nasabah group by rt');
  ...
  ?>
    ...
    series: [{
        ...
        $query = mysqli_query(
            $conn,
            "SELECT SUM(total) AS total FROM setor LEFT JOIN nasabah ON setor.nin= nasabah.nin where (tanggal_setor)<='" .
                date('Y-11-31') .
                "'  group by nasabah.rt"
        );
        while ($row = mysqli_fetch_array($query)) {
